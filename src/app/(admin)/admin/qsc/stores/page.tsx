...
const BUSINESS_TYPES = ["JOYFIT24", "JOYFIT LITE", "JOYFIT+", "FIT365", "FIT365 Premium"];
const COMPANIES = ["第1カンパニー", "第2カンパニー", "HQカンパニー"];
...
const MOCK: StoreRow[] = [
  { storeId: "S001", clubCode: 306, name: "札幌大通", brandName: "JOYFIT", businessTypeName: "JOYFIT24", companyName: "第1カンパニー", corporateName: "株式会社オカモト", status: "active", assetId: "A001", updatedAt: new Date().toISOString(), version: 1 },
  { storeId: "S002", clubCode: 565, name: "新宿西口", brandName: "FIT365", businessTypeName: "FIT365", companyName: "HQカンパニー", corporateName: "株式会社〇〇", status: "inactive", assetId: undefined, updatedAt: new Date().toISOString(), version: 2 },
];
...
export default function AdminStoresPage() {
  ...
  const [draft, setDraft] = useState<Partial<StoreRow>>({});
  ...
  const saveStore = () => {
    ...
    const next = { ...(draft as StoreRow), updatedAt: new Date().toISOString(), version: (draft.version || 0) + 1 };
    if (sheetMode === "create") {
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gap: 32 }}>
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={() => { setSheetMode("create"); setDraft({ brandName: "JOYFIT", status: "active" }); setSheetOpen(true); }} style={{ height: 48, padding: "0 24px", borderRadius: 16, background: "#1e293b", color: "#fff", border: "none", fontWeight: 900, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}><Plus size={22} />店舗を追加</button>
          </div>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 120, background: "rgba(15,23,42,0.4)", backdropFilter: "blur(10px)", display: "flex", justifyContent: "flex-end" }}>
          ...
          <div style={{ width: "100%", maxWidth: 600, background: "#fff", height: "100%", display: "grid", gridTemplateRows: "auto 1fr auto" }}>
            <div style={{ padding: "24px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ margin: 0, fontWeight: 950 }}>{sheetMode === "create" ? "新規登録" : "編集"}</h2>
            ...
            <div style={{ padding: "32px", overflowY: "auto", display: "grid", gap: 24 }}>
              ...
              {sheetMode === "edit" && (
                <button onClick={() => deleteStore(draft.storeId!)} style={{ background: "#fef2f2", color: "#ef4444", border: "1px solid #fee2e2", padding: "12px", borderRadius: 12, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><Trash2 size={16}/> 店舗を削除</button>
              )}
            </div>
            <div style={{ padding: "24px", background: "#f8fafc", borderTop: "1px solid #f1f5f9", display: "flex", gap: 12 }}>
              <button onClick={() => setSheetOpen(false)} style={{ flex: 1, height: 50, borderRadius: 12, border: "1px solid #e2e8f0", background: "#fff", fontWeight: 800 }}>キャンセル</button>
              <button onClick={saveStore} style={{ flex: 2, height: 50, borderRadius: 12, background: "#1e293b", color: "#fff", fontWeight: 900 }}>保存する</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(15,23,42,0.6)", backdropFilter: "blur(8px)", display: "grid", placeItems: "center" }}>
          <div style={{ background: "#fff", width: 400, borderRadius: 24, padding: "32px", display: "grid", gap: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <button onClick={() => setBatchModalOpen(false)} style={{ height: 46, borderRadius: 12, border: "1px solid #e2e8f0", background: "#fff", fontWeight: 800 }}>戻る</button>
              <button onClick={applyBatchAsset} style={{ height: 46, borderRadius: 12, background: "#4f46e5", color: "#fff", fontWeight: 900 }}>適用する</button>
            </div>
          </div>
        </div>
      )}
