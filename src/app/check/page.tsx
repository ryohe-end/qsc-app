...
function CompanyPicker({
  ...
  return (
    <>
      <button
        type="button"
        className={`qsc-companyBtn ${value ? "is-selected" : ""}`}
        onClick={() => setOpen(true)}
        aria-label="企業名を選択"
      >
        <span className="qsc-companyBtnLeft">
          <span className="qsc-companyBtnIcon">
            <Building2 size={18} />
          </span>
          <span className="qsc-companyBtnText">
            {value ? value.name : "企業名を選択"}
          </span>
        </span>

        <span className="qsc-companyBtnRight">
      ...
      {open ? (
        <div className="qsc-sheetOverlay" role="dialog" aria-modal="true">
          <button
            type="button"
            className="qsc-sheetBackdrop"
            onClick={() => setOpen(false)}
            aria-label="閉じる"
          />
          <div className="qsc-sheet">
            <div className="qsc-sheetHandle" />
            <div className="qsc-sheetHead">
              <div className="qsc-sheetTitle">企業名を選択</div>
              <button
                type="button"
                className="qsc-sheetClose"
                onClick={() => setOpen(false)}
                aria-label="閉じる"
              >
                <X size={18} />
            ...
            <div className="qsc-sheetSearch">
              <div className="qsc-sheetSearchIcon">
                <Search size={16} />
              </div>
              <input
              ...
              {q ? (
                <button
                  type="button"
                  className="qsc-sheetSearchClear"
                  onClick={() => setQ("")}
                  aria-label="検索をクリア"
                >
                  ×
            ...
            <div className="qsc-sheetList" role="listbox" aria-label="企業候補">
              <button
                type="button"
                className={`qsc-sheetOpt ${!value ? "is-active" : ""}`}
                onClick={() => {
                  onChange("");
                  setOpen(false);
                  setQ("");
              ...
              >
                <span className="qsc-sheetOptDot" />
                <span className="qsc-sheetOptText">（未選択）</span>
                {!value ? (
                  <span className="qsc-sheetOptCheck">
                    <Check size={18} />
                  </span>
                ) : (
                  <span />
              ...
              {filtered.map((o) => {
                ...
                return (
                  <button
                    key={o.id}
                    type="button"
                    className={`qsc-sheetOpt ${active ? "is-active" : ""}`}
                    onClick={() => {
                      onChange(o.id);
                      setOpen(false);
                      setQ("");
                  ...
                  >
                    <span className="qsc-sheetOptDot" />
                    <span className="qsc-sheetOptText">{o.name}</span>
                    {active ? (
                      <span className="qsc-sheetOptCheck">
                        <Check size={18} />
                      </span>
              ...
              {filtered.length === 0 ? (
                <div className="qsc-sheetEmpty">該当する企業がありません</div>
              ) : null}
            </div>
          </div>
        </div>
