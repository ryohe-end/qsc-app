...
export default function LoginPage() {
  ...
  // ✅ Logo fade (scroll じゃなく focus に連動)
  ...
  useEffect(() => {
    ...
    const ensureScript = () => {
      ...
      s.onerror = () => {
        setErrorMsg(
          "Googleログインの読み込みに失敗しました（ネットワーク/拡張機能を確認）"
        );
      };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMsg(null);
    setLoading(true);
    document.documentElement.classList.add("qsc-leave");
    ...
    try {
      ...
      document.documentElement.classList.remove("qsc-leave");
    } finally {
      window.setTimeout(() => {
        document.documentElement.classList.remove("qsc-leave");
      }, 300);
        ...
        <section className="qsc-card qsc-login-card qsc-enter qsc-enter--d4">
          ...
          <div
            style={{
              ...
              background:
                "linear-gradient(90deg, #2f8ce6 0%, #2fb36d 52%, #ff3b30 100%)",
          ...
          <form onSubmit={onSubmit} className="qsc-form" aria-label="login">
            ...
            {/* ✅ ログインボタン（スピナー） */}
            <button
              className="qsc-btn qsc-btn-primary"
              type="submit"
              disabled={!canSubmit}
            >
            ...
            <div style={{ display: "grid", placeItems: "center", gap: 10 }}>
              ...
              {googleBusy ? (
                <div style={{ fontSize: 12, opacity: 0.7 }}>Googleで認証中…</div>
              ) : null}
            </div>
          </form>
        </section>

        <footer className="qsc-footer qsc-enter qsc-enter--d5">
          © 2026 QSC Check
        </footer>
      </div>
    </main>
